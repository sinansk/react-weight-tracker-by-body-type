...
const AddNewRecordModal = () => {
  ...
  const handleUpdate = async ({ values }) => {
    ...
    await dispatch(
      setInput({ reduxName: "personalInfo", name: "height", value: height })
    );
    await dispatch(
      setInput({ reduxName: "personalInfo", name: "weight", value: weight })
    );
                  ...
                  <Field
                    ...
                    className={`${
                      errors.date && touched.date
                        ? "border-red-500"
                        : "border-[hsl(0,0%,80%)] "
                    } text-black w-full px-4 font-thin rounded outline-none lg:h-[38px] border-[1px] focus:border-2 focus:border-blue-400`}
                    type="date"
                    name="date"
                      ...
                      {Object.entries(measure).map(([key, value]) => (
                        <li key={key}>
                          ...
                          <div className="relative flex items-center">
                            <Field
                              ...
                              className={`${
                                errors[key] && touched[key]
                                  ? "border-red-500"
                                  : "border-[hsl(0,0%,80%)] "
                              }  w-full px-4 font-thin rounded outline-none lg:h-[38px] border-[1px] focus:border-2 focus:border-blue-400`}
                              type="number"
                              name={key}
                              onChange={handleChange}
                              value={values[key]}
                            ...
                            <BodyPartTooltip
                              bodyPart={key}
                              className="absolute top-0 bottom-0 right-0 flex items-center justify-center h-full px-3"
                            />
                          </div>
                        </li>
                      ))}
