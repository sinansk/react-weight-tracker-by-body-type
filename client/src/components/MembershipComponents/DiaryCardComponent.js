...
const DiaryCardComponent = ({
  ...
  const diary = userDiary.calorieDiary.find(
    (entry) => entry.date === selectedDate
  );
  const [selectedOption, setSelectedOption] = useState("");
  const calorieDiary = useSelector((state) => state.userDiary.calorieDiary);
  const foods = diary?.foods;
  const calorieRoutines = useSelector(
    (state) => state.userDiary.calorieRoutines
  );
  const columns = [
  ...
  const addCustomFood = () => {
    if (
      quickCalorie.food.calories !== undefined &&
      quickCalorie.food.calories !== ""
    ) {
  ...
  return (
    <>
      <div
        className={`${className} p-1 sm:p-5 text-xs sm:text-base pt-0 mx-auto shadow-lg rounded-xl text-gray-200  bg-gray-500/50 w-[600px] h-fit sm:max-h-[700px] overflow-auto no-scrollbar`}
      >
        <div className="relative flex items-center">
          <h2 className="p-2 m-auto text-lg" onClick={handleCalendarExpand}>
            {selectedDate}
          </h2>
          {diary && (
            <button
              ...
              className="float-right hover:text-teal-500"
            >
              <AiFillSave className="w-5 h-5 sm:w-8 sm:h-8" />
            </button>
          )}
        </div>
        {foods ? (
          <table className="min-w-full p-5 divide-y divide-gray-100">
            ...
            <tbody className="divide-y divide-gray-400 ">
              {foods?.map((item, rowIndex) => (
                <tr key={rowIndex} className="">
                  {columns.map((column) => (
                    <td
                      className="px-1.5 py-1 sm:px-3 sm:py-2 "
                      key={column.id}
                      title={column.id}
                    >
                      {column.id !== "food_name" ? (
                  ...
                  <td className="">
                    <DeleteButton
                      onClick={() =>
                        deleteDailyCalorie(item, calorieDiary, selectedDate)
                      }
                      className="w-3 h-3 m-auto sm:h-5 sm:w-5"
                    />
                  </td>
                </tr>
              ))}
              <tr
                className={`${
                  isQuickCalorie
                    ? `table-row outline-dashed outline-2  outline-offset-1 `
                    : `hidden`
                }`}
              >
                  ...
                  <td key={nutrient}>
                    <div className="relative">
                      <input
                        ...
                        placeholder={
                          nutrient === "calories" ? "required" : "optional"
                        }
                ...
                <td>
                  ...
                  >
                    <TbSquareRoundedCheckFilled
                      size={18}
                      className="m-auto text-teal-500 cursor-pointer hover:text-teal-400"
                    />
                  </button>
                </td>
              </tr>
            ...
            {diary && (
              <tfoot className="font-bold">
                <tr>
                  <td onClick={handleAddButton} colSpan={2}>
                    <span className="flex flex-row items-center justify-center px-3 py-1.5  text-teal-500 rounded-lg cursor-pointer hover:bg-gray-400/40">
                      <IoIosAdd size={20} /> Add Quick Calorie
                    </span>
                  </td>
                  <td title="Total Carbs">
                    {diary?.totalNutrient?.totalCarbs}
                  </td>
                  <td title="Total Fat">{diary?.totalNutrient?.totalFat}</td>
                  <td title="Total Protein">
                    {diary?.totalNutrient?.totalProtein}
                  </td>
                  <td title="Total Calories">
                    {diary?.totalNutrient?.totalCalories}
                  </td>
                  <td />
                </tr>
              </tfoot>
