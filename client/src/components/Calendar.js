...
const Calendar = ({ className, diaryDates, onDateClick, showContextMenu }) => {
  ...
  const [selectedDate, setSelectedDate] = useState(
    calendarDate ? calendarDate : moment()
  );
  ...
  const handleCopyFromDate = () => {
    setFromDate(
      calorieDiary?.find(
        (diaryItem) => diaryItem?.date === contextMenuDate?.format("DD-MM-YYYY")
      ) ?? null
    );
  };
  useEffect(() => {}, [contextMenuDate]);
    ...
    createModal("SaveRoutineModal", {
      onSubmit: async (values) => {
        await saveDiaryAsRoutine({
          uid,
          selectedDate: moment(contextMenuDate).format("DD-MM-YYYY"),
          foods: calorieDiary?.find(
            (diaryItem) =>
              diaryItem?.date === contextMenuDate?.format("DD-MM-YYYY")
          )?.foods,
          totalNutrient: calorieDiary?.find(
            (diaryItem) =>
              diaryItem?.date === contextMenuDate?.format("DD-MM-YYYY")
          )?.totalNutrient,
  ...
  const contextMenuButtons = [
    {
      ...
      icon: <AiFillCopy className="w-5 h-5 copy-diary" />,
      onClick: handleCopyFromDate,
      disabled: !isDateInDiary(contextMenuDate),
    },
    {
      label: "Copy to date",
      icon: <AiFillCheckSquare className="w-5 h-5" />,
      onClick: handleCopyToDate,
      disabled: !fromDate,
    },
    {
      label: "Delete diary",
      icon: <AiFillDelete className="w-5 h-5" />,
      onClick: handleDeleteFromDate,
      disabled: !isDateInDiary(contextMenuDate),
    },
    {
      label: "Save as routine",
      icon: <AiFillSave className="w-5 h-5 set-routine" />,
  ...
  const renderCalendarCell = (date) => {
    ...
    return (
      <td
        ...
        className={`cursor-pointer  hover:bg-gray-200/30 rounded-sm relative ${
          isContextMenuDate && "bg-gray-200/30"
        } ${isActive ? " bg-cyan-400 hover:bg-cyan-400 text-white" : ""}`}
        onClick={() => handleDateClick(date)}
      >
        <div
          className={`absolute bottom-[0.5px] right-0 left-0 z-10 ${
            isInDiary
              ? " border-b-[3px] rounded-t-md rounded-b-none border-green-500"
              : ""
          }`}
        ></div>
        <div
          className={`relative ${isToday ? "border-2  border-teal-500" : ""}`}
  ...
  const blankDays = Array.from(
    { length: firstDayOfMonth.weekday() },
    (_, index) => <td key={`blank-${index}`} className="0" />
  );
  ...
  const lastBlankDays = Array.from(
    { length: lastBlankDaysCount },
    (_, index) => <td key={`blank-${index + totalSlots}`} className="" />
  );
  ...
  return (
    ...
    >
      <button
        ...
        className="absolute top-0 right-0 px-2.5 py-1 text-xs border-2 border-teal-500 rounded-lg sm:top-2 sm:right-2 text pink-500 hover:bg-teal-500 hover:text-white"
      >
        TODAY
      </button>
      <div className="flex items-center justify-between mt-2 mb-4 sm:mt0 sm:w-full sm:px-20">
        <button
          onClick={(e) => handlePrevMonthClick(e)}
        ...
        <button
          onClick={(e) => handleNextMonthClick(e)}
          className="px-2 py-1 text-sm font-semibold "
        >
          {">"}
        </button>
      ...
      {showContextMenu && (
        <ContextMenu
          ...
          contextMenuDate={
            contextMenuDate && moment(contextMenuDate).format("DD-MM-YYYY")
          }
