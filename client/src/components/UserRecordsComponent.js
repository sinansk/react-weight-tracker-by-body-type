...
const UserRecordsComponent = () => {
  ...
  const TableHeader = ({ columns }) => {
    return (
      <thead className="sticky top-0 left-0 right-0 mt-4 shadow-md bg-slate-50">
        <tr>
          {columns.map((column) => (
            <th
              key={column.id}
        ...
        <tbody className="bg-white divide-y divide-gray-200 ">
          {transformedData?.map((item, index) => (
            <tr className="" key={item.id}>
              ...
              <td className="sticky left-0 px-6 py-4 whitespace-nowrap bg-slate-50">{moment(item.data.date * 1000).format("DD MMM YYYY")}</td>
